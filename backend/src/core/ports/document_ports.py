...
from core.repo.document_repo import INSERT_DOCUMENT, GET_DOCUMENT_BY_ID, DELETE_DOCUMENTS_BY_PROCESS, \
    ...
    UPDATE_DOCUMENT_TEMPLATE, GET_DOCUMENT_TEMPLATE_BY_TYPE_ID, GET_DOCUMENT_TEMPLATES_BY_FLAG
...
class DocumentPorts:
    ...
    def get_all_document_templates(is_report: bool = None) -> list:
        if is_report is not None:
            return adapter.fetch_list(GET_DOCUMENT_TEMPLATES_BY_FLAG, (is_report,))
    ...
    def update_document_template(
        ...
        adapter.execute_query(
            ...
            (file_content, file_name, file_size, mime_type, is_report, document_type_id)
        )
    ...
    def save_document_template(
        ...
        adapter.execute_query(
            INSERT_DOCUMENT_TEMPLATE,
            (document_type_id, file_content, file_name, file_size, mime_type, is_report)
        )

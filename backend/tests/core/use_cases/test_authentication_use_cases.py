...
@pytest.mark.asyncio
async def test_login_calls_authorize_redirect_and_returns_response(dependencies: dict, mock_request: object) -> None:
...
@pytest.mark.asyncio
async def test_auth_stores_user_and_access_token_and_returns_dashboard_url(dependencies: dict, mock_request: object) -> None:
    token = {
    ...
    dependencies["oauth_provider"].authorize_access_token.assert_awaited_once_with(mock_request)
...
@pytest.mark.asyncio
async def test_auth_handles_missing_token_fields(dependencies: dict, mock_request: object) -> None:
    token = {}
    expected_url = "http://localhost:5173/dashboard"
...
def test_logout_pops_user_and_access_token(dependencies: dict, mock_request: object) -> None:
...
def test_current_user_returns_session_user(dependencies: dict, mock_request: object) -> None:
